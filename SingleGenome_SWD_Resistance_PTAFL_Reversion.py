...
Modified on Thur Nov 18 11:26:30

@author: Adam Sychla

Block release of multiple EGI

python 2021-10-05_SingleGenome_SWD_Cleaned_AS.py timeStepAdded ReleaseNum Run Type Blocks

"""
windows=False #if True imports winsound and alerts end of simulation. Set to false if NOT running on Windows
...
#argvs python (program) (releaseFreq) (releaseSize) (SaveLoc) (EGIType) (releaseBlock)
...
LM.append([0.5,1,0.5,0.5,1,0.5,0.5])#D locus
LM.append([0.5,0.5,1,0.5,0.5,0.5,0.5])#P locus
LM.append([0.5,0.5,0.5,1,0.5,1,0.5])#T locus
LM.append([0.5,1,0.5,0.5,1,0.5,0.5])#L locus
LM.append([0.5,0.5,0.5,1,0.5,1,0.5])#G locus
...
#Allows for X linked GE
    startingGMOgenotypeB =  'BBDDppttLLWWXX'     # SGD= 'bbddppttllGWXX', SSIMS = 'bbddPPTTLLWWXX', EGI = 'bbddPPTTllWWXX', FL = 'bbddppttLLWWXX', FAFL = bbdppttllWWUU
    startingGMOgenotypeR =  'bbddPPTTllFFXX'     # SGD= 'bbddppttllGWXX', SSIMS = 'bbddPPTTLLWWXX', EGI = 'bbddPPTTllWWXX', FL = 'bbddppttLLWWXX', FAFL = bbdppttllWWUU
    startingGMOgenotypeP =  'BBDDPPTTLLFFXX'     # SGD= 'bbddppttllGWXX', SSIMS = 'bbddPPTTLLWWXX', EGI = 'bbddPPTTllWWXX', FL = 'bbddppttLLWWXX', FAFL = bbdppttllWWUU
    wildtypeGenotype = 'bbddppttllWWXX'         # for most simulations, wt = 'bbddppttllWWXX' NOTE:'bbddpp' is added further in the code to allow resistance
    startingGMOgenotypeF = 'bbddppttLLWWXX'		#FL genotype
    ...
    #Resistance rates
    Freq1 = 0.01                       #Rate of SNP1 in population
    Freq2 = 0.01                       #Rate of SNP2 in population
    genSNP = 31                     #Timestep at which to create SNPs
    fail=0.1						#failure rate of the female lethal constructs
    fail2=0.1						#failure rate of the PTA
    ...
    releaseCount = 0
    timeSteps = 220#len(TempData)                     # number of time steps to simulate
    migrationRate = 0                   # range [0-1]; movement per adult per timestep;
    
    #Secondary Model parameters
    startingWT = 1000          # wild-type agents placed at time step 0
    ...
    DayEggsPerFemale = 60
    MaxEggsPerFemale = 150

    carry=False # how much tet to raise the flies on. True==100mg/mL False==10mg/mL

    typeExp = sys.argv[4]+str(startingGMO)+'start'+str(timeStepAdded)+'Release'+str(addedGMO)+'Block'+sys.argv[5]              # This is used for Filename generation '[typeExp]experiment[#].pkl'    
    ...
    class SWD(Agent):
        ...
        def mature(self):                                   # Progression from one life=stage to the next at each time step
            ...
            if self.state == 'egglv':
                ...
                elif 'F' in self.genotype and self.tet==False and self.sex == 'female': #Comment out '''and self.sex == 'female' ''' for RIDL           #kill by female lethality
                    self.lifeStage = 10000
                    self.state = 'dead'
                    self.model.grid._remove_agent(self.pos, self)
                    self.model.schedule.remove(self)

                elif 'p' in self.genotype and 'T' in self.genotype:   #kill if wild-type promoter present with PTA
                    self.state = 'dead'
                    self.lifeStage = 10000
                    self.model.grid._remove_agent(self.pos, self)
                    self.model.schedule.remove(self)
                ...
                if random.random()<1/2*(1+math.erf((y-93.22)/(6.18*math.sqrt(2)))):
                    ...
                    self.state = 'adult'
                    self.lifeStage = 0
                    self.model.totAdults+=1
                    if self.sex=='female':
                        self.model.totFAdults+=1
                    if 'bbddppttllWW' in self.genotype:
                        self.model.totWtAdults+=1
                        if self.sex=='female':
                            self.model.totFWtAdults+=1

                    if 'pr' in self.genotype or 'rp' in self.genotype:
                        self.model.totARresHet+=1
                    if 'bc' in self.genotype or 'cb' in self.genotype:
                        self.model.totABresHet+=1
                    if 'rr' in self.genotype:
                        self.model.totARres+=1
                    if 'cc' in self.genotype:
                        self.model.totABres+=1
            ...
            if female.state == 'adult':
                ...
                for i in range(Eggs2Lay):
                    ...
                    if sSelect>=0.3:
                        ...
                        genSelect=1
                    if genSelect>=len(maleGenotype):
                        genSelect=len(maleGenotype)-1
                    ...
                    if 'L' in haploM and random.random()<fail:
                        haploM=haploM.replace('L','l')
                    if 'F' in haploM and random.random()<fail:
                        haploM=haploM.replace('F','W')
                    if 'T' in haploM and random.random()<fail2:
                        haploM=haploM.replace('T','t')
                    if 'D' in haploM and random.random()<fail2:
                        haploM=haploM.replace('D','d')
                    ...
                    if 'L' in haploF and random.random()<fail:
                        haploF=haploF.replace('L','l')
                    if 'F' in haploF and random.random()<fail:
                        haploF=haploF.replace('F','W')
                    if 'T' in haploF and random.random()<fail2:
                        haploF=haploF.replace('T','t')
                    if 'D' in haploM and random.random()<fail2:
                        haploF=haploF.replace('D','d')
                    #/generate Maternal Allele to pass on
                    ...
                    if 'Pp' in egg.genotype or 'pP' in egg.genotype:
                        if random.random() < promoterConversionFreq:
                            egg.genotype = egg.genotype.replace('p','q')

                    if 'Bb' in egg.genotype or 'bB' in egg.genotype:
                        if random.random() < promoterConversionFreq:
                            egg.genotype = egg.genotype.replace('b','z')
                    ...
                    self.model.tot+=1
                    if 'bbddppttllWW' in egg.genotype:
                        self.model.totWt+=1
                    if 'pr' in egg.genotype or 'rp' in egg.genotype:
                        self.model.totRresHet+=1
                    if 'bc' in egg.genotype or 'cb' in egg.genotype:
                        self.model.totBresHet+=1
                    if 'rr' in egg.genotype:
                        self.model.totRres+=1
                    if 'cc' in egg.genotype:
                        self.model.totBres+=1
        ...
        def female_mate(self):                                                  # Mating-ready females find random mating partner male
            ...
            if len(cellmates) > 1:
                ...
                if len(matingPartners) > 0:
                    ...
                    if len(self.mateGenotype)==3:
                        del self.mateGenotype[0]
        ...
        def step(self):
            self.mature()
            if i == genSNP:
                if 'pp' in self.genotype:
                    if random.random()<Freq1:
                        self.genotype = self.genotype.replace('pp','rp')
                    if random.random()<Freq1:
                        self.genotype = self.genotype.replace('pp','pr')
                        self.genotype = self.genotype.replace('rp','rr')
                else:
                    if random.random()<Freq1:
                        self.genotype = self.genotype.replace('p','r')

                if 'bb' in self.genotype:
                    if random.random()<Freq2:
                        self.genotype = self.genotype.replace('bb','cb')
                    if random.random()<Freq2:
                        self.genotype = self.genotype.replace('bb','bc')
                        self.genotype = self.genotype.replace('cb','cc')
                else:
                    if random.random()<Freq2:
                        self.genotype = self.genotype.replace('b','c')
    ...
    filename = sys.argv[3]+sys.argv[4] + '_Release' + sys.argv[5] + 'Block'+ sys.argv[1]+'RelFreq'+sys.argv[2]+'RelTot'+'.pkl'
    filename2 = sys.argv[3]+sys.argv[4] + '_Release' + sys.argv[5] + 'Block'+ sys.argv[1]+'_Totals.csv'
    ...
    for i in range(gridHeight):
        for j in range(gridWidth):
            for h in range(startingWT):
                ...
                new.model.tot+=1
                new.model.totWt+=1
                new.model.totWtAdults+=1
                new.model.totAdults+=1
                if 'XX' in new.genotype:
                    new.model.totFAdults+=1
                    new.model.totFWtAdults+=1
    ...
    with Timer('FULL EXECUTION'):     
        releaseCount = 0
		#NOTE: Flipped genotypes beacause they are flipped again at the first release
        if sys.argv[4] == 'P':
            startingGMOgenotype=startingGMOgenotypeP
        elif sys.argv[4] == 'B':
            startingGMOgenotype=startingGMOgenotypeR
        elif sys.argv[4] == 'R':
            startingGMOgenotype=startingGMOgenotypeB
        elif sys.argv[4] == 'F':
            startingGMOgenotype=startingGMOgenotypeF
		
        for i in range(timeSteps):

            dfTemp = pd.DataFrame([[i,model.tot, model.totWt, model.totAdults, model.totFAdults, model.totWtAdults, model.totFWtAdults,model.totBresHet,model.totABresHet,model.totBres,model.totABres,model.totRresHet,model.totARresHet,model.totRres,model.totARres]],
                       columns=['Step','Total', 'Wt_Total', 'Adults','Female_Adults','Wt_Adults','Female_Wt_Adults','BresHet','BresHet_Adults','Bres','Bres_Adults','RresHet','RresHet_Adults','Rres','Rres_Adults'])

            dfTotals=pd.concat([dfTotals,dfTemp])

            # Adding new GE organisms and saving compressed datafile at each multiple of timestep added
            if i > 32 and i % timeStepAdded == 0:


                #if P nothing happens, if D flips but is reset later, if R or B it flips
                if (releaseCount % int(sys.argv[5]) == 0) and (releaseCount > 0):
					
                    if startingGMOgenotype == startingGMOgenotypeR:
                        startingGMOgenotype = startingGMOgenotypeB
                    elif startingGMOgenotype == startingGMOgenotypeB:
                        startingGMOgenotype = startingGMOgenotypeR
                ...
                for j in range(gridHeight-(2*borderCells)):
                    for k in range(gridWidth-(2*borderCells)):
                        ...
                        for l in range(addedGMO):
                            ...
                            # reset to random choice of R or B for each agent
                            if sys.argv[4] == 'D':
